...
public class Configuration
{
    ...
    public Amadeus build() => new(parseEnvironment());
    ...
    public Configuration setSsl(bool ssl)
    {
        this.ssl = ssl;
        if (!ssl && port == 443)
        {
    ...
    {
        _ = setHostname(getOrDefault("HOSTNAME", hostname));
        host = getOrDefault("HOST", host);
        logLevel = getOrDefault("LOG_LEVEL", logLevel);
        _ = setSsl(Boolean.Parse(getOrDefault("SSL", ssl.ToString())));
    ...
    {
        var value = Environment.GetEnvironmentVariable("AMADEUS_" + key);
